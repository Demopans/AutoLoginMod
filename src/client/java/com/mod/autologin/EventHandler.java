...
public class EventHandler {
	...
	public void onActionPerformed(ActionPerformedEvent.Pre ev) {
		if (ev.getButton() instanceof GuiAlButton)
			((GuiAlButton) ev.getButton()).resetSizeFactor();
		if (ev.getGui() instanceof GuiScreenAddServer && ev.getButton().equals(loginButton)) {
			try {
				String server = serverAddress(ev.getGui());
				LoginData logAddress = ModMain.getLoginData(server, true);
				mc.displayGuiScreen(new GuiPasswordBox(logAddress, ev.getGui(), server, data -> ModMain.saveConfigs()));
			} catch (Exception e) {
				e.printStackTrace();
			}
			ev.setCanceled(true);
	...
	public void onChatMessage(ClientChatReceivedEvent ev) {
		if (!isConnected && !mc.isSingleplayer() && mc.getCurrentServerData() != null) {
			if (ev.getMessage().getUnformattedText().toLowerCase()
					.contains(currentLoginData.cloneEmpty().getLoginMessage().trim())) {
				if (currentLoginData.getType() == LoginDataType.SECURE_LOGIN)
					mc.displayGuiScreen(
							new GuiPasswordBox(currentLoginData.cloneEmpty(), null, mc.getCurrentServerData().serverIP,
									data -> mc.player.sendChatMessage(data.getLoginMessage())));
					...
					mc.player.sendChatMessage(currentLoginData.getLoginMessage());
			} else if (ev.getMessage().getUnformattedText().toLowerCase()
					.contains(currentLoginData.cloneEmpty().getRegisterMessage().trim()))
				if (currentLoginData.getType() == LoginDataType.SECURE_LOGIN)
					mc.displayGuiScreen(
							new GuiPasswordBox(currentLoginData.cloneEmpty(), null, mc.getCurrentServerData().serverIP,
									data -> mc.player.sendChatMessage(data.getRegisterMessage())));
				else if (currentLoginData.emptyPassword())
					mc.displayGuiScreen(new GuiPasswordBox(currentLoginData, null, mc.getCurrentServerData().serverIP,
							data -> mc.player.sendChatMessage(data.getRegisterMessage())));
				else
					mc.player.sendChatMessage(currentLoginData.getRegisterMessage());
			isConnected = true;
		}
	}
	...
	public void onInitGui(InitGuiEvent.Post ev) {
		if (ev.getGui() instanceof GuiScreenAddServer) {
			for (Object obj : ev.getButtonList()) {
				GuiButton button = (GuiButton) obj;
				switch (button.id) {
				case 0: // done
					loginButton.x = button.x;
					loginButton.y = button.y;
					button.y = ev.getGui().height / 4 + 120 + 18;
					button.x = ev.getGui().width / 2 + 2;
					button.width = 98;
					break;
				case 1: // cancel
					button.width = 98;
			...
			ev.getButtonList().add(loginButton);
		}
	}
