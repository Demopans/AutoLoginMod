...
public class GuiPasswordField extends GuiTextField {
	...
	public void drawTextBox() {
		...
		if (lock) {
			...
			drawTexturedModalRect(this.x + this.width + 3, this.y + this.height / 2 - 10, 0, 146, 20,
					20);
			this.width += 26;
		}
		setText(text);
	...
	public boolean textboxKeyTyped(char typedChar, int keyCode) {
		...
		if (isFocused() && GuiScreen.isKeyComboCtrlX(keyCode)) { // CUT
			writeText("");
			return true;
		}
		return !GuiScreen.isKeyComboCtrlC(keyCode) && super.textboxKeyTyped(typedChar, keyCode); // COPY
	}
}
